...
def make_ai():
    ...
    if config.ai_backend == "openai":
        return ai.OpenAIAI(model_id=config.ai_model_id, api_key=config.openai_api_key)
    if config.ai_backend == "groq":
        return ai.GroqAI(model_id=config.ai_model_id, api_key=config.groq_api_key)
    if config.ai_backend == "ollama":
        return ai.OllamaAI(model_id=config.ai_model_id)
