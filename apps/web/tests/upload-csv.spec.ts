...
const CSV_FILE = path.resolve('../../apps/api/sample_data/bank_statement_q2_2026.csv')
...
test('Upload real CSV file — fresh DB', async ({ page }) => {
  ...
  await page.screenshot({ path: shot('upload_01_overview_empty.png'), fullPage: true })

  // 2. Go to import page
  await page.goto('http://localhost:3000/app/import')
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(500)
  await page.screenshot({ path: shot('upload_02_import_ready.png'), fullPage: true })
  ...
  await expect(page.locator('text=/Đang tải|Uploading/i')).toBeVisible({ timeout: 5_000 })
  await page.screenshot({ path: shot('upload_03_processing.png'), fullPage: true })

  // Wait for "Hoàn tất" or "Complete"
  await expect(page.locator('text=/Hoàn tất|Complete/i')).toBeVisible({ timeout: 15_000 })
  await page.waitForTimeout(800)
  await page.screenshot({ path: shot('upload_04_complete.png'), fullPage: true })
  ...
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(1500)
  await page.screenshot({ path: shot('upload_05_transactions.png'), fullPage: true })

  const txBody = await page.locator('body').innerText()
  const hasTx = /Highlands|Salary|payroll|Grab|Apple|Netflix|Airlines/i.test(txBody)
  ...
  await page.screenshot({ path: shot('upload_07_overview_data.png'), fullPage: true })

  // 8. Spending analysis
  await page.goto('http://localhost:3000/app/spending')
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(1500)
  await page.screenshot({ path: shot('upload_08_spending.png'), fullPage: true })

  const captured = fs.readdirSync(SCREENSHOTS)
...
test('Use sample CSV button — after DB reset', async ({ page }) => {
  ...
  const resetRes = await page.request.delete('http://localhost:8000/transactions', {
  ...
  // 3. Wait for progress
  await expect(page.locator('text=/Đang tải|Uploading/i')).toBeVisible({ timeout: 5_000 })
  await page.screenshot({ path: shot('sample_02_processing.png'), fullPage: true })

  await expect(page.locator('text=/Hoàn tất|Complete/i')).toBeVisible({ timeout: 15_000 })
  await page.waitForTimeout(800)
  await page.screenshot({ path: shot('sample_03_complete.png'), fullPage: true })

  // 4. Transactions
  await page.goto('http://localhost:3000/app/transactions')
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(1000)
  await page.screenshot({ path: shot('sample_04_transactions.png'), fullPage: true })

  // 5. Overview
  await page.goto('http://localhost:3000/app/overview')
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(1500)
  await page.screenshot({ path: shot('sample_05_overview.png'), fullPage: true })
