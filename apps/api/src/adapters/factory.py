...
def make_ai():
    ...
    if config.ai_backend == "ollama":
        return ai.OllamaAI(url=config.ollama_url, model_id=config.ai_model_id)
...
def make_userstore():
    ...
    if backend == "postgres":
        if not config.userstore_postgres_url:
            print("WARNING: USERSTORE_POSTGRES_URL is empty, falling back to sqlite")
